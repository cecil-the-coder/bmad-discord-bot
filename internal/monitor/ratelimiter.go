...
type ProviderRateLimitState struct {
	ProviderID          string                 // e.g., "ollama", "openai", "claude"
...
func (rm *RateLimitManager) getProviderStatusLocked(provider *ProviderRateLimitState) string {
	...
	if provider.DailyQuotaExhausted {
		...
		if time.Now().After(provider.DailyQuotaResetTime) {
			// This scenario should ideally be handled by the calling service (e.g., OllamaAIService)
...
// NewRateLimiter creates a new rate limiter instance with default Ollama configuration
